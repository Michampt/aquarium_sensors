...
class PowerDevice:
    def __init__(self, ip_addr: str = None, user: str = None) -> None:
    ...
    def init_power(self):
        try:
            logging.info("Loading current power states")
            self.check_power_states(False)
            logging.info(f"Fan: {self.fan_power_state}")
            logging.info(f"Heater: {self.heater_power_state}")
            ...
            if changed_initial_state:
                logging.info("Loading new power states")
                logging.info(f"Fan: {self.fan_power_state}")
                logging.info(f"Heater: {self.heater_power_state}")
